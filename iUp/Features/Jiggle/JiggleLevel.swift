...
/// `.full` is effectively unbounded — the cursor wanders across the display.
enum JiggleLevel: String, CaseIterable, Identifiable {
    ...
    /// `.full` returns an effectively unbounded radius so the walk roams
    /// freely (bounded only by the display edges).
    var radius: CGFloat {
        switch self {
        case .slightest: 1
        case .low:       10
