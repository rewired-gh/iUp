...
- **Release packaging.** `ditto -c -k --keepParent build/Release/iUp.app /tmp/iUp-vX.Y.Z.zip` preserves the code-signature for distribution. Upload/overwrite asset: `gh release upload vX.Y.Z /tmp/iUp-vX.Y.Z.zip --clobber`. `gh auth login` is interactive (device-flow only — cannot be automated; user must open github.com/login/device with the one-time code).
- **No confirmation prompts when intent is clear.** Proceed directly with commit, push, release upload, etc. Only pause for genuinely ambiguous or non-obvious destructive actions.
